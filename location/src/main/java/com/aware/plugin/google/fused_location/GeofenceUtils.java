...
class GeofenceUtils {
    ...
    static String getLabel(Context context, Location currentLocation) {
    ...
    static float getDistance(Location a, Location b) {
        float[] distance = new float[2];
        Location.distanceBetween(a.getLatitude(), a.getLongitude(), b.getLatitude(), b.getLongitude(), distance);
        return distance[0]/1000;
    }

    static Location getLabelLocation(Context context, String label) {
    ...
    static int getLabelLocationRadius(Context context, String label) {

        int radius = 1;
        Cursor labels = getLabels(context, label);

        if (labels != null && labels.moveToFirst()) radius = labels.getInt(labels.getColumnIndex(Provider.Geofences.GEO_RADIUS));

        if (labels != null && ! labels.isClosed()) labels.close();
        return radius;
    }

    static void saveLabel(Context context, String label, Location currentLocation, double radius) {
    ...
    static Cursor getLabels(Context context, @Nullable String filter_label) {
